...
class TopologyNTN(Topology):
    ...
    def __init__(
        ...
        self.space_station_x = (
            self.bs_radius * np.cos(self.bs_elevation) * np.cos(self.bs_azimuth)
        )
        self.space_station_y = (
            self.bs_radius * np.cos(self.bs_elevation) * np.sin(self.bs_azimuth)
        )
    ...
    def calculate_coordinates(self, random_number_gen=np.random.RandomState()):
        ...
        if self.num_sectors == 19:
            ...
            for k in range(6):
                ...
                self.x.append(
                    d * np.cos(np.radians(angle)) + d * np.cos(np.radians(angle + 60))
                )
                self.y.append(
                    d * np.sin(np.radians(angle)) + d * np.sin(np.radians(angle + 60))
                )
        ...
        distance_xy = np.sqrt(
            (self.x_rotated - self.space_station_x) ** 2
            + (self.y_rotated - self.space_station_y) ** 2
        )
        self.elevation = (
            np.arctan2(self.z - self.space_station_z, distance_xy) * 180 / np.pi
        )
    ...
    def plot(self, axis: matplotlib.axes.Axes):
        ...
        for x, y in zip(self.x / 1000, self.y / 1000):  # Convert to kilometers
            ...
            for a in range(6):
                ...
                hexagon.append(
                    [x + r * math.cos(angle_rad), y + r * math.sin(angle_rad)]
                )
    ...
    def plot_3d(self, axis: matplotlib.axes.Axes, map=False):
        ...
        if map:
            ...
            federal_district_coords_km = (
                federal_district_coords[0] * lon_to_km,
                federal_district_coords[1] * lat_to_km,
            )
        ...
        axis.plot(
            ...
            [0, self.space_station_z / 1000],
            "b-",
            label=f"Height = {self.space_station_z /
                              1000:.1f} km",
        )
        ...
        axis.plot(
            ...
            [0, self.space_station_z / 1000],
            "g--",
            label=f"Slant range = {self.bs_radius /
                                   1000:.1f} km",
        )
