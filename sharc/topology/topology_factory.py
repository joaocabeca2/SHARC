...
class TopologyFactory(object):
    ...
    def createTopology(parameters: Parameters) -> Topology:
        if parameters.imt.topology.type == "SINGLE_BS":
            return TopologySingleBaseStation(parameters.imt.topology.single_bs.cell_radius, parameters.imt.topology.single_bs.num_clusters)
        elif parameters.imt.topology.type == "MACROCELL":
            return TopologyMacrocell(parameters.imt.topology.macrocell.intersite_distance, parameters.imt.topology.macrocell.num_clusters)
        elif parameters.imt.topology.type == "HOTSPOT":
            return TopologyHotspot(parameters.imt.topology.hotspot, parameters.imt.topology.hotspot.intersite_distance, parameters.imt.topology.hotspot.num_clusters)
