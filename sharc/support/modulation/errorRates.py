import numpy as np
import sympy
from numpy import log2, sqrt, sin, pi
...
def psk_awgn(M: int, gamma_s: float) -> any:
    """
    Generate PSK Symbol error rate
    ...
    M : int
        Modulation level
    gamma_s : float
        Energy per modulation symbol to noise spectral density

    Returns
    -------
    np.array
        The simbol error rates
    """
...
def qam_awgn(M, gamma_s):
    """
    Generate QAM Symbol error rate
    ...
    if (M == 1) or (np.mod(np.log2(M), 2) != 0):  # M not a even power of 2
        raise ValueError(
            'Only square MQAM supported. M must be even power of 2')
...
def pam_awgn(M, gamma_s):
    """
    Generate PAM Symbol error rate
...
def fsk_awgn(M_val: int, gamma_s_vals: list, coherence: str) -> np.array:
    """
    Generate FSK Symbol error rate
    ...
    coherence : str
        'coherent' for coherent FSK detection
        'noncoherent' for noncoherent FSK detection
    ...
    if coherence.lower() == 'coherent':
        ...
        M, i = sympy.symbols('M i', integer=True, positive=True)
        gamma = sympy.Symbol('gamma')
        s = sympy.Sum((-1)**(i+1)/(i+1)*sympy.binomial(M-1, i)
                * sympy.exp(-i/(i+1)*gamma), (i, 1, M-1))
        for j, gamma_s in enumerate(gamma_s_vals):
            # evaluate the expression with values for M and gamma_s
            SERs[j] = s.evalf(subs={M: M_val, gamma: gamma_s})
    else:
        raise ValueError(
            'For FSK coherence must be \'coherent\' or \'noncoherent\'')
    return SERs
