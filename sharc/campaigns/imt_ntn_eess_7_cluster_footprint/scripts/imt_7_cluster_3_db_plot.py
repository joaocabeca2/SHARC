...
def generate_eess():
    ntn_topology = TopologyMacrocell(param_mss.intersite_distance,
                                    7
    )
    num_bs = 100
    mss_ss = StationManager(n=num_bs)
    mss_ss.station_type = StationType.MSS_SS
    mss_ss.x = np.ones(num_bs) + param_mss.x
    mss_ss.y = np.ones(num_bs) + param_mss.y
    mss_ss.height = np.ones(num_bs)
    mss_ss.elevation = 18
...
if __name__ == "__main__":
    ...
    ntn_topology = TopologyMacrocell(param_mss.intersite_distance, 7)
