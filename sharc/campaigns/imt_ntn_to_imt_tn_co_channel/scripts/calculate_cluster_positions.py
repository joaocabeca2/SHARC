# Helper script that calculates the coordintaes of the NTN footprint based
# on the distance between borders with IMT-TN
...
if __name__ == "__main__":
    ...
    ntn_footprint_x_offset = macro_topology_radius + \
        ntn_footprint_radius + border_distances_array
    ...
    macro_topology = TopologyMacrocell(
        intersite_distance, macrocell_num_clusters)
    macro_topology.calculate_coordinates()

    ntn_circle = plt.Circle(
        (param_mss.x,
         0),
        radius=ntn_footprint_radius,
        fill=False,
        color='green')
    macro_cicle = plt.Circle(
        (0, 0), radius=macro_topology_radius, fill=False, color='red')
    ...
    for i, d in enumerate(border_distances_array):
        ...
        print(
            f"Satellite elevation w.r.t to boresight = {
                param_mss.elevation} deg")
        print(
            f"Satellite azimuth w.r.t to boresight = {
                param_mss.azimuth} deg")
        print(f"Border distance = {d / 1000} km")
        print(
            f"NTN nadir offset w.r.t. IMT-TN cluster center = {ntn_footprint_x_offset[i] / 1000} km")
        print(
            f"Satellite elevation w.r.t. IMT-TN cluster center = {
                np.round(
                    np.degrees(
                        np.arctan(
                            param_mss.altitude /
                            ntn_footprint_x_offset[i])))} deg")
        slant_path_len = np.sqrt(
            param_mss.altitude**2 +
            ntn_footprint_x_offset[i]**2)
        print(
            f"Slant path lenght w.r.t. IMT-TN cluster center = {slant_path_len / 1000} km")
        fspl = np.round(
            20 *
            np.log10(slant_path_len) +
            20 *
            np.log10(
                param_mss.frequency *
                1e6) -
    ...
    ax.annotate(
        ...
        ((macro_topology_radius + border_distances_array[dist_idx] / 2),
         1000))
