...
if __name__ == "__main__":
    ...
    orbit_positions = {0: {'x': [], 'y': [], 'z': []}, 1: {'x': [], 'y': [], 'z': []}}
    ...
    for _ in range(NUM_DROPS):
        ...
        idx_orbit = mss_d2d_manager.idx_orbit

        # Store positions by orbit
        for orbit_id in np.unique(idx_orbit):
            orbit_mask = np.where(idx_orbit == orbit_id)
            orbit_positions[orbit_id]['x'].append(x_vec[orbit_mask])
            orbit_positions[orbit_id]['y'].append(y_vec[orbit_mask])
            orbit_positions[orbit_id]['z'].append(z_vec[orbit_mask])

        # Identify visible satellites
        vis_sat_idxs = np.where(mss_d2d_manager.active)[0]
        visible_positions['x'].append(x_vec[vis_sat_idxs])
        visible_positions['y'].append(y_vec[vis_sat_idxs])
        visible_positions['z'].append(z_vec[vis_sat_idxs])

    # Concatenate arrays for each orbit
    for orbit_id in orbit_positions:
        orbit_positions[orbit_id]['x'] = np.concatenate(orbit_positions[orbit_id]['x'])
        orbit_positions[orbit_id]['y'] = np.concatenate(orbit_positions[orbit_id]['y'])
        orbit_positions[orbit_id]['z'] = np.concatenate(orbit_positions[orbit_id]['z'])

    # Concatenate visible satellite positions
    ...
    # Plot satellites for each orbit
    orbit_colors = {0: 'red', 1: 'yellow'}
    for orbit_id, color in orbit_colors.items():
        fig.add_trace(go.Scatter3d(
            x=orbit_positions[orbit_id]['x'].flatten(),
            y=orbit_positions[orbit_id]['y'].flatten(),
            z=orbit_positions[orbit_id]['z'].flatten(),
            mode='markers',
            marker=dict(size=2, color=color, opacity=0.5),
            name=f'Orbit {orbit_id + 1}'
        ))

    # Plot visible satellites (green markers)
    fig.add_trace(go.Scatter3d(
        x=visible_positions['x'].flatten(),
        y=visible_positions['y'].flatten(),
        z=visible_positions['z'].flatten(),
        mode='markers',
