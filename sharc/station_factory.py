    ...
    def generate_imt_ue_outdoor(param: ParametersImt,
        ...
        if param.ue_distribution_type.upper() == "UNIFORM" or \
           ...
           param.ue_distribution_type.upper() == "UNIFORM_IN_CELL":
            ...
            elif param.ue_distribution_distance.upper() == "SQRT(UNIFORM)":
                radius = topology.cell_radius * \
    ...
    def generate_wifi_earth_station(param: ParametersWifiEs, random_number_gen: np.random.RandomState, *args):
        ...
        if param.location.upper() == "FIXED":
            ...
            if param.min_dist_to_bs < 0:
                sys.stderr.write("ERROR\nInvalid minimum distance from wifi ES to BS: {}".format(
...
#        haps.y = np.array([0, 9*h, 15*h, 6*h, -9*h, -15*h, -6*h])
    ...
    def get_random_position(num_stas: int, topology: Topology,
        ...
        while len(x) < num_stas:
            ...
            x_temp[invert_index] = (
                hexagon_radius * np.cos(np.pi / 6) - x_temp[invert_index])
        ...
        x_old = x
        x = cell_x + x * \
            np.cos(azimuth_rad[cell]) - y * np.sin(azimuth_rad[cell])
        y = cell_y + x_old * \
            np.sin(azimuth_rad[cell]) + y * np.cos(azimuth_rad[cell])
