        ...
        if "LAT_LONG_INSIDE_COUNTRY" in params.sat_is_active_if.conditions:
            ...
            if len(country_proj) > 1:
                raise ValueError(
                    ...
                    f"appears {len(country_projgeometry)} times in shapefile {params.sat_is_active_if.lat_long_inside_country.country_shapes_filename}"
                )
            ...
            for orbit_idx, param in enumerate(params.orbits):
                ...
                for condition in params.sat_is_active_if.conditions:
                    if "MINIMUM_ELEVATION_FROM_ES" == condition:
                        ...
                        polygon_mask = sats_points.within(country_proj)
                        ...
                        if params.sat_is_active_if.lat_long_inside_country.margin_from_border != 0.0:
                            ...
                            flat_lon = np.repeat(
                                flat_lon, len(country_border_coords_lon)
                            ).reshape(
                                (len(flat_lon), len(country_border_coords_lon))
                            )
                            flat_lat = np.repeat(
                                flat_lat, len(country_border_coords_lon)
                            ).reshape(
                                (len(flat_lon), len(country_border_coords_lon))
                            )

                            border_lon = np.resize(
                                country_border_coords_lon,
                                len(flat_lon) * len(country_border_coords_lon)
                            ).reshape(
                                (len(flat_lon), len(country_border_coords_lon))
                            )
                        ...
                        raise NotImplementedError(
                            ...
                            f"{params.sat_is_active_if.condition} is not a recognized value for that"
                        )
