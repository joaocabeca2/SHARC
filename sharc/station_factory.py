...
class StationFactory(object):
    ...
    def generate_imt_base_stations(
        ...
        for i in range(num_bs):
            imt_base_stations.antenna[i] = AntennaBeamformingImt(
                param_ant, imt_base_stations.azimuth[i], imt_base_stations.elevation[i]
            )
    ...
    def generate_imt_ue_outdoor(
        ...
        if param.ue_distribution_type.upper() == "UNIFORM":
            ...
            if type(topology) is not TopologyMacrocell:
                sys.stderr.write(
    ...
    def generate_fss_space_station(param: ParametersFssSs):
        ...
        imt_long_diff_rad = param.earth_station_long_diff_deg * np.pi / 180.0
        ...
        imt_lat_rad = param.earth_station_lat_deg * np.pi / 180.0
        fss_space_station.x = (
    ...
    def get_random_position(
        ...
        # calculate UE azimuth wrt serving BS
        if not topology.is_space_station:
            ...
            distance = np.sqrt(
                (cell_x - x) ** 2 + (cell_y - y) ** 2 + (topology.bs_height) ** 2
            )
