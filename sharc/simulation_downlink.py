...
class SimulationDownlink(Simulation):
    ...
    def calculate_sinr_ext(self):
        ...
        ue_ext_int = ext_interference - self.coupling_loss_imt_system[active_sys, :][ :, ue]
       
        # Sum all the interferers for each UE
        self.ue.ext_interference[ue] = 10*np.log10(np.sum(np.power(10, 0.1*ue_ext_int), axis=0))
