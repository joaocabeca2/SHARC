...
class ParametersNTN(ParametersBase):
    ...
    def load_parameters_from_file(self, config_file: str):
        ...
        if not np.all((0 <= self.bs_azimuth) & (self.bs_azimuth <= 360)):
            raise ValueError("ParametersNTN: bs_azimuth values must be between 0 and 360 degrees")

        if not np.all((0 <= self.bs_elevation) & (self.bs_elevation <= 90)):
            raise ValueError("ParametersNTN: bs_elevation values must be between 0 and 90 degrees")
