...
class ParametersIndoor(ParametersBase):
    ...
    # Possible values: "FSPL" (free-space path loss), "INH_OFFICE" (3GPP
    ...
    def load_parameters_from_file(self, config_file: str):
        ...
        if self.basic_path_loss.upper() not in ["FSPL", "INH_OFFICE"]:
            raise ValueError(
                f"ParamtersIndoor: Invalid topology name {
        ...
        if self.num_imt_buildings.upper() != "ALL":
            if self.num_imt_buildings.isnumeric():
                ...
                raise ValueError(
                    f"ParamtersIndoor: Invalid topology name {
                        self.num_imt_buildings}"
                )

        if self.building_class.upper() not in ["TRADITIONAL", "THERMALLY_EFFICIENT"]:
            raise ValueError(
                f"ParametersIndoor: Inavlid Spectral Mask Name {
                    self.building_class}"
            )

        if not (0 <= self.ue_indoor_percent or self.ue_indoor_percent <= 1):
            raise ValueError(
                f"ParametersIndoor: ue_indoor_percent must be between 0 and 1, but is {
                    self.ue_indoor_percent}"
            )

        # Ensure the number of rows is greater than zero
        if self.n_rows <= 0:
            raise ValueError(
                f"ParametersIndoor: n_rows must be greater than 0, but is {
                    self.n_rows}"
            )
        ...
        if self.n_colums <= 0:
            raise ValueError(
                f"ParametersIndoor: n_colums must be greater than 0, but is {
                    self.n_colums}"
            )
