...
class PropagationEHF(Propagation):
    ...
    def get_gaseous_attenuation(self, distance: np.ndarray,
                                frequency: np.ndarray,
                                indoor_stations: np.ndarray,
                                elevation: np.ndarray,
                                tx_gain: np.ndarray,
                                rx_gain: np.ndarray) -> np.ndarray:
...
if __name__ == '__main__':
    ...
    n = np.asarray(2.0 * np.ones(distance_2D.shape))

    indoor_stations = np.asarray(np.zeros(distance_3D.shape, dtype=bool))
    tx_gain = np.asarray(np.ones((1, 1), dtype=float) * 0)
    rx_gain = np.asarray(np.ones((1, 1), dtype=float) * 0)
    elevation = np.asarray(np.zeros(distance_3D.shape, dtype=bool))
    
    ehf = PropagationEHF(random_number_gen)
    free_space_loss = ehf.get_free_space_loss(frequency * 1000, distance_3D)
    ...
    loss = ehf._get_loss(frequency, distance_3D, ref_dist, free_space_loss, gas_att, rain_att, n)
