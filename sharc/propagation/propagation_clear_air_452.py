...
class PropagationClearAir(Propagation):
    ...
    def __init__(
        self, random_number_gen: np.random.RandomState, model_params: ParametersP452
    ):
    ...
    def get_loss(
        ...
        frequency_array = (
            frequency * np.ones(distance.shape) * (1e-3)
        )  # P.452 expects GHz
        ...
        return self.get_loss(
            distance, frequency_array, indoor_stations, elevation, tx_gain, rx_gain
        )
    ...
    def get_loss(
        ...
        for ii in range(num_dists):
            ...
            Lbs = self.tl_tropo(
                dtot, theta, frequency, p[ii], T, Ph, N0, tx_gain[ii], rx_gain[ii]
            )
