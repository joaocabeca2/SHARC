...
class PropagationP619(Propagation):
    ...
    def _get_atmospheric_gasses_loss(self, *args, **kwargs) -> float:
        ...
        surf_water_vapour_density = kwargs.pop("surf_water_vapour_density", None)

        memo_key = (frequency_MHz, apparent_elevation, surf_water_vapour_density)

        if memo_key in self.memoized_params:
            return self.memoized_params[memo_key]
        ...
        def get_next_layer_params(h, rho_s, frequency_MHz, delta, beta, r):
            ...
            if m >= 0:
                ...
                return dh, gamma, None, None, None
            ds = (r + delta) * np.cos(beta) - np.sqrt((r + delta)**2 * np.cos(beta)**2 - (2 * r * delta + delta**2))  # slope distance
            ...
            n = n_new
            return ds, gamma, h, delta, beta, r
        ...
        if apparent_elevation < 0:
            ...
            while True:
                ds, gamma, h, delta, beta, r = get_next_layer_params(h, rho_s, frequency_MHz, delta, beta, r)
                a_acc += ds * gamma
                if h is None:
                    break

        _, _, _, n, gamma = self.atmosphere.get_atmospheric_params(h, rho_s, frequency_MHz)
        ...
        while True:
            ...
            if h >= 100:
                break
            r += delta
            _, _, _, n_new, gamma = self.atmosphere.get_atmospheric_params(h, rho_s, frequency_MHz)
            beta = np.arcsin(n / n_new * np.sin(alpha))
            n = n_new

        self.memoized_params[memo_key] = a_acc
...
def main():
    ...
    loss_2_5= propagation._get_atmospheric_gasses_loss(frequency_MHz=frequency_MHz,
    ...
    index = 0
    attenuation= propagation._get_beam_spreading_att(elevation_vec,
                                                    altitude_vec[index],
                                                    earth_to_space)
...
if __name__ == '__main__':
    ...
    # Analisar as estatísticas e salvá-las em um arquivo CSV
    s = StringIO()
    ps = pstats.Stats(pr, stream=s).sort_stats('cumulative')
    ps.print_stats()
    
    # Extrair as estatísticas e salvá-las em um CSV
    stats = ps.stats

    with open('profile_stats_gasses.csv', 'w', newline='') as csvfile:
        fieldnames = ['Function', 'Calls', 'Total Time', 'Per Call', 'Cumulative Time', 'Per Call Cumulative']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for func_desc, func_data in stats.items():
            writer.writerow({
                'Function': f"{func_desc[2]} ({func_desc[0]}:{func_desc[1]})",
                'Calls': func_data[0],
                'Total Time': func_data[2],
                'Per Call': func_data[2] / func_data[0] if func_data[0] != 0 else 0,
                'Cumulative Time': func_data[3],
                'Per Call Cumulative': func_data[3] / func_data[0] if func_data[0] != 0 else 0
            })