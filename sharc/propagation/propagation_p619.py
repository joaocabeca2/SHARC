...
class PropagationP619(Propagation):
    ...
    def __init__(
        ...
        if season.upper() not in ["SUMMER", "WINTER"]:
            raise ValueError(
                f'PropagationP619: Invalid value for parameter season - {
    ...
    def _get_atmospheric_gasses_loss(self, *args, **kwargs) -> float:
        ...
        if lookupTable and self.city_name != "Unknown":
            ...
            csv_file = os.path.join(
                output_dir,
                f"{self.city_name}_{int(frequency_MHz)}_{
    ...
    def apparent_elevation_angle(
        cls, elevation_deg: np.array, space_station_alt_m: float
    ) -> np.array:
        ...
        tau_fs_deg = 1 / (
            tau_fs1 + space_station_alt_m * tau_fs2 + space_station_alt_m**2 * tau_fs3
        )
    ...
    def get_loss(
        ...
        is_intra_imt = (
            True if station_a.is_imt_station() and station_b.is_imt_station() else False
        )
        if is_intra_imt:
            # Intra-IMT
        ...
        # FIXME: Need this hack because the other IMT propagtion models returns
        # num_bs x num_ue
        if is_intra_imt:
    ...
    def get_loss(
        ...
        atmospheric_gasses_loss = self._get_atmospheric_gasses_loss(
            frequency_MHz=freq_set, apparent_elevation=np.mean(elevation["apparent"])
        )
        beam_spreading_attenuation = self._get_beam_spreading_att(
            elevation["free_space"], self.earth_station_alt_m, earth_to_space
        )
        diffraction_loss = 0
        ...
        if single_entry:
            ...
            building_loss = (
                self.building_entry.get_loss(frequency, elevation["apparent"])
                * indoor_stations
            )
