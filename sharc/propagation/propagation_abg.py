...
class PropagationABG(Propagation):
    ...
    def get_loss(
        ...
        """
        wrap_around_enabled = (
            params.imt.wrap_around
            and (params.imt.topology == "MACROCELL" or params.imt.topology == "HOTSPOT")
            and params.imt.num_clusters == 1
        )

        if wrap_around_enabled:
            _, bs_to_ue_dist_3d, _, _ = station_b.get_dist_angles_wrap_around(station_a)
        else:
    ...
    def get_loss(
        ...
        loss = (
            10 * self.alpha * np.log10(distance)
            + self.beta
            + 10 * self.gamma * np.log10(frequency * 1e-3)
            + shadowing
            + building_loss
        )
...
if __name__ == "__main__":
    ...
    abg_los = abg.get_loss(
        distance_2D, freq, np.zeros(shape=distance_2D.shape, dtype=bool), False
    )
