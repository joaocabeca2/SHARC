...
class PropagationClutterLoss(Propagation):
    ...
    def get_terrestrial_clutter_loss(
        ...
        # minimum path length for the correction to be applied at only one end
        # of the path
...
if __name__ == "__main__":
    ...
    for i, _ in enumerate(loc_percentage):
        clutter_loss[:, i] = cl.get_spacial_clutter_loss(
            frequency_mhz, elevation_angle, loc_percentage[i]
        )
    ...
    plt.title(
        f"Cumulative distribution of clutter loss not exceeded for {
            frequency_mhz[0] /
            1000:.2f} GHz"
    )
    ...
    for i in range(len(frequency_mhz)):
        clutter_loss_ter[i, :] = cl.get_terrestrial_clutter_loss(
            frequency_mhz[i] * np.ones(distance.shape),
            distance,
            loc_percentage,
            apply_both_ends,
        )
