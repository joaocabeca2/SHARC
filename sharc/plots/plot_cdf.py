...
def plot_cdf(
    ...
    csv_folder = os.path.abspath(
        os.path.join(workfolder, "..", "campaigns", base_dir, "output")
    )
    figs_folder = os.path.abspath(
        os.path.join(workfolder, "..", "campaigns", base_dir, "output", "figs")
    )
    ...
    if subfolders:
        ...
        subdirs = [
            ...
            if os.path.isdir(os.path.join(csv_folder, d))
            and d.startswith(f"output_{base_dir}_")
        ]
    ...
    for subdir in subdirs:
        all_files = [
            ...
            if f.endswith(".csv") and label in f and file_prefix in f
        ]
    ...
    for idx, subdir in enumerate(subdirs):
        ...
        legenda = (
            legends[idx]
            if legends
            else os.path.basename(subdir).split(f"output_{base_dir}_")[1]
        )
...
if __name__ == "__main__":
    ...
    all_plots(
        name, legends=legends, subfolders=subfolders, save_file=True, show_plot=False
    )
